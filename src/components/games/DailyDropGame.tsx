...
import { getDailyQuestions, generateDynamicDaily } from '../../data/questions';
...
export const DailyDropGame: React.FC<DailyDropGameProps> = ({
  profile,
  onBackToLobby,
  questionsExplored,
  onIncrementExplored
  ...
  const [questions, setQuestions] = useState<string[]>(() => getDailyQuestions(language));
  const dayIndex = Math.floor(Date.now() / (1000 * 60 * 60 * 24)) % (questions.length || 1);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onGameSync((data) => {
      if (data.gameKey !== 'daily-drop') return;

      if (data.action === 'DAILY_NEXT' || data.action === 'ROUND_RESET') {
  ...
  const handleNext = () => {
    ...
    if (nextIdx >= questions.length) {
      const dyn = generateDynamicDaily(nextIdx + 4, language);
      setQuestions(prev => [...prev, dyn]);
    }

    if (isOnlineMode) {
      sendGameAction('DAILY_NEXT', {
        gameKey: 'daily-drop',
        nextIndex: nextIdx
      });
    } else {
  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center max-w-2xl mx-auto px-4 py-8 relative z-10">
      <motion.div
        key={question}
      ...
      >
        ...
        <div className="flex items-center justify-between pb-3 border-b border-[#2D2D2D]/8 text-xs text-[#6E6664]">
          <span className="font-serif italic">{language === 'id' ? 'Pertanyaan hari ini' : 'Daily question'}</span>
                ...
                {!myLocked ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-xs text-[#8E2435] font-semibold">
                      ...
                      <span className="text-[#2D2D2D]/50 text-[11px]">{language === 'id' ? 'Bebas & tulus' : 'Open & honest'}</span>
                    </div>
                    <textarea
                      rows={3}
                      value={myAnswer}
                      onChange={(e) => setMyAnswer(e.target.value)}
                      placeholder={language === 'id' ? 'Tuliskan ceritamu hari ini...' : 'Write your daily update...'}
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <span className="text-xs font-semibold text-[#8E2435] bg-[#FAF7F2] px-3 py-1 rounded-full border border-[#8E2435]/15">
                    {language === 'id' ? '✨ Jawaban Harian Terbuka' : '✨ Daily Answers Revealed'}
                  </span>
                </div>
